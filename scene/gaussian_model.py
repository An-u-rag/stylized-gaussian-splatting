...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        self._features_dc = torch.empty(0)  # Correspond to color features
        self._features_rest = torch.empty(0)  # Correspond to color features
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        print("PCD Points Shape: ", fused_point_cloud.shape) # [N, 3]
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        print("PCD Colors Shape: ", fused_color.shape) # [N, 3]
        # Color and Spherical Harmonics feature initialization - features
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda() 
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0 # ???????
        ##########################
        print("Features Shape: ", features.shape) # [N, 3, 16]
        ...
        # May need to modify opacities for stylization
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
        
        # Initialization of optimizable parameters
