...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, style_image=None, stylize_iter_start=None):
    ...
    if checkpoint:
        ...
        print("Loaded Iteration: ", first_iter)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Inverse Rendering - Compare rendered output to Ground Truth viewpoint
        # Loss
        gt_image = viewpoint_cam.original_image.cuda() 
        Ll1 = l1_loss(image, gt_image) 
        # CONTENT LOSS - L1 Loss + SSIM Loss
        ...
        if style_image!=None and iteration > w_s_start_iter:
            ...
            encoder(image.unsqueeze(0).to(style_device))
            for sl in style_losses:
                style_loss += sl.loss
            w_s = w_s * w_s_mult_rate

            if iteration % 1000 == 0:
                print("Style Weight: ", w_s)
        
        loss = (w_c * content_loss) + (w_s * style_loss)
        loss.backward()
...
if __name__ == "__main__":
    ...
    # Read Style Image and load into tensor
    style_image = None
    if args.style_path:
        ...
        loader = transforms.Compose([
            ...
            transforms.ToTensor()])
        style_image = loader(style_image).cuda() # [3, 1277, 848]
